...
from src.partialconv2d import PConvBNActiv
...
class VectorQuantizer(nn.Module):
    ...
    def update_semantic_label(self):
        ...
        if self.n_cluster > 0:
            if self.semantic_label is None:
                ...
                l_f = torch.randint(0, self.n_cluster, size=(self.n_e - self.n_cluster, 1)).squeeze()#后 n_e-n_cluster 个嵌入向量的语义标签为[0,self.n_cluster-1]之间的随机整数。
                self.semantic_label = torch.cat((l_c, l_f), dim=0).to(self.device)#将所有语义标签拼接在一起，保存到类变量self.semantic_label中
            else:
                # with torch.no_grad:
                d = self.get_distance(self.embed_weight[self.n_cluster:])#计算后(n_e-n_cluster)个嵌入向量与所有n_cluster个聚类中心之间的距离矩阵
                label_ = torch.argmax(0 - d[:, :self.n_cluster], dim=1)#取出每个后(n_e-n_cluster)个嵌入向量与聚类中心距离最近的那个聚类中心的编号，保存在label_中
    ...
    def get_distance(self, z, code_type='all'):
        ...
        if self.distance_type == 'euclidean':# 当距离类型为欧几里得距离时
            ...
            raise NotImplementedError('distance not implemented for {}'.format(self.distance_type)) # 抛出异常，提示尚未实现该距离度量类型
    ...
    def _quantize(self, z, token_type=None, topk=1, step=None, total_steps=None):
        ...
        #？ min_encoding_indices = torch.zeros(z.shape[0]).long().to(z.device)   # 创建一个全部元素为 0 的一维张量
            ...
            token_semantic_type = likelihood * torch.mean(embed_semantic_label, dim=0)  # P(k|z) = P(z|k) * P(k) # 计算 token 的语义类型
        ...
        if self.get_embed_type == 'matmul':
            ...
            z_q = F.embedding(min_encoding_indices, self.embed_weight)  # .view(z.shape)  # 将最相似的码本向量索引传入 F.embedding 函数，然后得到最终的量化向量 z_q
        else:
            raise NotImplementedError
